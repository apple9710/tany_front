...
// 업로드 타입 확인
$type = getParam('type');
if (!$type || !in_array($type, ['reference', 'resource', 'certification', 'history', 'main_product'])) {
    sendError('type은 reference, resource, certification, history, main_product 중 하나여야 합니다', 400);
}
...
switch ($type) {
    ...
    case 'main_product':
        $uploadDir = $uploadBaseDir . '/main_products';
        $allowedTypes = ['jpg', 'jpeg', 'png', 'gif', 'webp'];
        $maxSize = 10485760; // 10MB
        break;
}
